...
def check_test_acc(loader, model, noise=0.00):
    ...
    for data, labels in loader:
        ...
        outputs_og = model(data)
        _, pred_og = torch.max(outputs_og, 1)
        correct += (pred_og == labels).float().sum(0).item()

        total += data.size(0)

    print(f"Noise-Sigma={noise} | {(correct/total)*100.:.2f}")
...
if __name__ == "__main__":
    ...
    loader = DataLoader(dataset, shuffle=False, batch_size=100, num_workers=2)

    # load the base classifier
    if args.base_classifier in IMAGENET_CLASSIFIERS:
        assert args.dataset == "imagenet"
        # loading pretrained imagenet architectures
        base_classifier = get_architecture(
            args.base_classifier, args.dataset, pytorch_pretrained=True
        )
    else:
    ...
    print(f"Evaluation With-Out Denoiser")
    check_test_acc(loader, base_classifier, noise=args.sigma)
    ...
    print(f"Evaluation With Denoiser")
    check_test_acc(loader, base_classifier, noise=args.sigma)
    ...
    if not os.path.exists(args.outfile):
        print(f"Creating Directory @ {args.outfile}")
        os.makedirs(args.outfile)

    args.outfile = f'{args.outfile}{args.denoiser.split("/")[-2]}'
    print(f"Ceritfying @ {args.outfile} for sigma={args.sigma}")

    assert args.sigma == float(args.denoiser.split("/")[-2].split("_")[-1])
