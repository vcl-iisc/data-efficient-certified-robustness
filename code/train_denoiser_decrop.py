...
from disc_arch import DomainDisc
...
parser.add_argument("--w_alpha", help="weight for alpha", default=1.0, type=float)
parser.add_argument("--disc_lr", help="lr for disc.", default=1e-4, type=float)
...
args.outdir = f"./denoiser_models/{args.per_cls_per:.2f}_neg_domain_disc_{args.aug}_mixcoeff_{args.mix_coeff}_obj_{args.w_ce}*{args.objective}_{args.w_cs}*cosim_pgd_inter_{args.w_mmd}*mmd_pgd_inter_clf_{arch}_{args.outdir}_{args.arch}_{args.noise_sd}"
...
def main():
    ...
    domain_disc = DomainDisc().cuda()

    optimizer_domain = torch.optim.Adam(domain_disc.parameters(), lr=args.disc_lr)
    criterion_domain = torch.nn.NLLLoss().cuda()

    for epoch in range(starting_epoch, args.epochs):
        before = time.time()
        train_loss, domain_loss, domain_acc = train(
            train_loader,
            denoiser,
            criterion,
            optimizer,
...
def train(
    ...
    domain_disc: torch.nn.Module = None,
    ...
    for i, (inputs, inputs_adv, inputs_inter, targets) in enumerate(loader):
        ...
        p = float(i + epoch * len(loader)) / args.epochs / len(loader)
        alpha = 2.0 / (1.0 + np.exp(-10 * p)) - 1

        alpha = alpha * args.w_alpha
        ...
        ## Apply domain discriminatibility loss
        input_domain = torch.cat((inputs, denoiser(inputs + noise)), 0)
        _, features = classifier(input_domain)
        domain_label = (
            torch.cat((torch.zeros(inputs.size(0)), torch.ones(inputs.size(0))), 0)
            .long()
            .cuda()
        )

        domain_output = domain_disc(
            features, alpha=alpha
        )  ## Predict domain disc features
        loss_domain = criterion_domain(domain_output, domain_label)

        _, domain_pred = torch.max(domain_output, 1)
        corr = (domain_pred == domain_label).float().sum(0).item()
        acc = (corr / domain_pred.size(0)) * 100.0
        domain_accuracy.update(acc, domain_pred.size(0))
        domain_loss.update(loss_domain.item(), domain_pred.size(0))
        ...
        loss_cosim_adv = args.w_cs * cosim_loss(denoised_adv_logits, targets_logits)
        loss_cosim_inter = args.w_cs * cosim_loss(denoised_inter_logits, targets_logits)

        loss_mmd_adv = args.w_mmd * mmd_loss(features_og, features_noisy_adv)
        loss_mmd_inter = args.w_mmd * mmd_loss(features_og, features_noisy_inter)
    ...
    return losses.avg, domain_loss.avg, domain_accuracy.avg
