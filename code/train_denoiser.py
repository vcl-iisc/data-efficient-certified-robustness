...
from train_utils import AverageMeter, accuracy, init_logfile, log, copy_code, requires_grad_, seed_everything
...
parser.add_argument('--per_cls_per', type=float, default=1.00, help='per-class sample percent for training data')
parser.add_argument('--wandb',help='Log Using WANBD', action='store_true')
parser.add_argument('--seed', type=int, default=3)
args = parser.parse_args()

arch = args.classifier.split('/')[-3].split('_')[0]
args.outdir = f'./denoiser_models/{args.per_cls_per:.2f}_obj_{args.objective}_clf_{arch}_{args.outdir}_{args.arch}_{args.noise_sd}'

print(f'Setting Seed as: {args.seed}')
seed_everything(args.seed)

if args.wandb:
    run = wandb.init(project='lim_cert_camera_ready',name=args.outdir.split('/')[-1], config=args, reinit=True)
...
def main():
    ...
    train_dataset = get_dataset(args.dataset, 'train', args.per_cls_per)
