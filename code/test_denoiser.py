...
def main(args):
    ...
    if args.test_subset:
        ...
        test_dataset, _ = torch.utils.data.random_split(
            test_dataset, [subset_len, len(test_dataset) - subset_len]
        )
    ...
    if args.clf != "":
        ...
        clf_loss, clf_acc = test_with_classifier(
            test_loader,
            denoiser,
        ...
        print(
            "Accuracy WITH denoiser at noise of {:.2f} is {}".format(
                args.noise_sd, clf_acc
            )
        )
        ...
        print(
            "Accuracy WITHOUT denoiser at noise of {:.2f} is {}".format(
                args.noise_sd, clf_acc
            )
        )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--denoiser", type=str, default="", help="Path to a denoiser ", required=True
    )
    parser.add_argument(
        "--clf",
        type=str,
        default="",
        help="Pretrained classificaiton model.",
        required=True,
    )
    parser.add_argument(
        "--outdir",
